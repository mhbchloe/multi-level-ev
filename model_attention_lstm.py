...
class AttentionLayer(nn.Module):
    ...
    def forward(self, lstm_outputs):
        ...
        # 计算注意力分数
        attention_scores = self.attention_weights(lstm_outputs)  # (batch, seq_len, 1)
        attention_scores = torch.softmax(attention_scores, dim=1)  # 归一化
        
        # 加权求和
        context = torch.sum(attention_scores * lstm_outputs, dim=1)  # (batch, hidden_dim)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 编码
        lstm_out, (hidden, cell) = self.encoder_lstm(x)  # lstm_out: (batch, seq_len, hidden_dim)
        
        # 应用注意力
        context, attention_weights = self.attention(lstm_out)  # context: (batch, hidden_dim)
        
        # 到潜在空间
        latent = self.encoder_fc(context)  # (batch, latent_dim)
        
        # 解码
        hidden_dec = self.decoder_fc(latent)  # (batch, hidden_dim)
        hidden_dec = hidden_dec.unsqueeze(1).repeat(1, seq_len, 1)  # (batch, seq_len, hidden_dim)
        
        decoded, _ = self.decoder_lstm(hidden_dec)
        output = self.output_fc(decoded)  # (batch, seq_len, input_dim)
        
        return output, latent, attention_weights
...
class AttentionLSTMTrainer:
    ...
    def __init__(self, input_dim, hidden_dim=64, latent_dim=16, num_layers=2, n_clusters=5):
        ...
        self.model = AttentionLSTM(input_dim, hidden_dim, latent_dim, num_layers).to(self.device)
        self.n_clusters = n_clusters
        self.scaler = StandardScaler()
        self.history = {'train_loss': [], 'val_loss': []}
        
        print(f"📐 Attention-LSTM参数: hidden={hidden_dim}, latent={latent_dim}, layers={num_layers}")
    ...
    def train(self, sequences, epochs=50, batch_size=32, lr=0.001, val_split=0.2):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                reconstructed, latent, attention = self.model(batch)
    ...
    def extract_features(self, sequences):
        ...
        with torch.no_grad():
            for batch in loader:
                batch = batch.to(self.device)
                _, latent, attention = self.model(batch)
                latent_features.append(latent.cpu().numpy())
                attention_weights_list.append(attention.cpu().numpy())
        
        self.attention_weights = np.vstack(attention_weights_list)  # 保存注意力权重用于可视化
    ...
    def visualize_attention(self, sample_idx=0, save_dir='./results/attention_lstm'):
        """可视化注意力权重（示例）"""
        os.makedirs(save_dir, exist_ok=True)
        
        if not hasattr(self, 'attention_weights'):
            print("⚠️  需要先运行extract_features")
            return
        
        # 取一个样本的注意力权重
        attention = self.attention_weights[sample_idx].squeeze()  # (seq_len,)
        
        plt.figure(figsize=(12, 4))
        plt.plot(attention, linewidth=2)
        plt.xlabel('Time Step')
        plt.ylabel('Attention Weight')
        plt.title('Attention Weights Visualization (Sample Event)')
        plt.grid(True, alpha=0.3)
        plt.savefig(f'{save_dir}/attention_visualization.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"📊 注意力可视化已保存")
    
    def visualize_training(self, save_dir='./results/attention_lstm'):
        """可视化训练"""
        os.makedirs(save_dir, exist_ok=True)
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('Attention-LSTM Training History')
        plt.legend()
        plt.grid(True, alpha=0.3)
    ...
    def save_model(self, save_dir='./results/attention_lstm'):
        """保存模型"""
        os.makedirs(save_dir, exist_ok=True)
        ...
        }, f'{save_dir}/attention_lstm_model.pth')
...
if __name__ == "__main__":
    ...
    trainer = AttentionLSTMTrainer(input_dim=6, hidden_dim=64, latent_dim=16, num_layers=2, n_clusters=5)
