...
def engineer_features(trips_df: pd.DataFrame, charging_df: pd.DataFrame) -> pd.DataFrame:
    ...
    注意：不能用
        features_df.columns = ['vehicle_id'] + [col for col in features_df.columns if col != 'vehicle_id']
    因为 features_dict 的 value 里没有 'vehicle_id'，该赋值会使新列标签数（N+1）与
    现有列数（N）不匹配，抛出 ValueError: Length mismatch。
    正确做法：先用 orient='index' 建 DataFrame（vehicle_id 成为行索引），
    再用 reset_index().rename(columns={'index': 'vehicle_id'}) 将索引变成列。
    """
    features_dict: dict = {}
    ...
    # ────────────────────────────────────────────────────────
    # 修复：使用 reset_index().rename() 而非直接赋值列标签
    # 直接赋值 features_df.columns = ['vehicle_id'] + [...] 会导致
    # ValueError: Length mismatch，因为新标签数（N+1）≠ 原列数（N）。
    # ────────────────────────────────────────────────────────
...
if ('anxiety_level' in features_df.columns
    ...
    if 'fast_charge_rate' in features_df.columns:
        ...
        mean_fcr = [features_df.loc[features_df['anxiety_level'] == lv,
                    ...
                    for lv in level_order]
        bar_colors = plt.cm.RdYlGn_r(np.linspace(0.15, 0.85, len(level_order)))
        ax.bar(level_order, mean_fcr, color=bar_colors, edgecolor='black', linewidth=1)
